...
import { FaTimes, FaCalendarAlt, FaUserCheck, FaWhatsapp, FaPhoneAlt, FaCheckCircle } from 'react-icons/fa';
...
const EnquiryModal = ({ isOpen, onClose, selectedPackage = '' }) => {
  ...
  const handleWhatsAppRedirect = () => {
    const text = `*New Event Booking Enquiry*\n\n` +
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md overflow-y-auto">
        <motion.div
          ...
          className="relative w-full max-w-lg bg-stone-900 border border-[#C9A227]/40 rounded-3xl p-6 sm:p-8 shadow-[0_15px_50px_rgba(0,0,0,0.9)] my-8"
        >
          {/* Close Button */}
          <button
            onClick={onClose}
            className="absolute top-4 right-4 text-stone-400 hover:text-white bg-stone-800 hover:bg-stone-700 w-8 h-8 rounded-full flex items-center justify-center transition-colors cursor-pointer"
          >
            <FaTimes />
          </button>
          ...
          {!submittedBooking ? (
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-4 text-left text-xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-stone-300 font-medium mb-1">Tentative Event Date</label>
                    <input
                      type="date"
                      name="date"
                      value={formData.date}
                  ...
                  <div>
                    <label className="block text-stone-300 font-medium mb-1">Preferred Time Slot</label>
                    <select
                      name="timeSlot"
                ...
                <div>
                  <label className="block text-stone-300 font-medium mb-1">Expected Guests</label>
                  <select
                    name="guests"
                ...
                <div className="flex items-center justify-center gap-4 text-[11px] text-stone-400 pt-2 border-t border-stone-800">
                  <span>Or connect instantly:</span>
                  <a
                    href={`https://wa.me/${info.whatsapp}?text=${encodeURIComponent('Hello! I would like to enquire about booking dates at Murugu Mahal.')}`}
                    target="_blank"
            ...
            <div className="text-center py-4 space-y-4">
              <div className="w-16 h-16 bg-[#C9A227]/20 border border-[#C9A227] text-[#C9A227] rounded-full flex items-center justify-center text-3xl mx-auto shadow-lg">
                <FaCheckCircle />
              </div>
              <div>
                ...
                <p className="text-xs text-stone-300 mt-1">
                  Your reference ID is <span className="text-[#C9A227] font-mono font-bold">{submittedBooking.id}</span>. Our reservation manager will contact you at <span className="text-stone-100 font-bold">{formData.phone}</span> within 2 hours.
                </p>
              </div>

              <div className="p-4 rounded-2xl bg-stone-950 border border-stone-800 text-left text-xs space-y-1">
                <div className="flex justify-between"><span className="text-stone-400">Name:</span> <span className="font-bold text-stone-200">{formData.name}</span></div>
                <div className="flex justify-between"><span className="text-stone-400">Date:</span> <span className="font-mono text-[#C9A227]">{formData.date || 'TBD'}</span></div>
                <div className="flex justify-between"><span className="text-stone-400">Event:</span> <span className="text-stone-200">{formData.eventType}</span></div>
                <div className="flex justify-between"><span className="text-stone-400">Status:</span> <span className="text-amber-400 font-semibold">Pending Review</span></div>
              </div>
              
              <div className="space-y-2.5 pt-2">
                <Button variant="primary" onClick={handleWhatsAppRedirect} className="w-full justify-center py-3 text-xs font-bold shadow-lg">
